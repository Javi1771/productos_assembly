...
export async function GET(req, { params }) {
  ...
  try {
    ...
    let tableTried = [];
    if (source === "usuarios" || source === "operadores") tableTried = [source === "usuarios" ? USERS_TABLE : OPERATORS_TABLE];
    else tableTried = [USERS_TABLE, OPERATORS_TABLE];

    for (const table of tableTried) {
...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    //! Si no se especifica tabla: intentar primero Usuarios, luego Operadores
...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    //? 2) Decidir tabla destino por rol
    const newTable = decideTargetTable(rol ?? (curTable===OPERATORS_TABLE?3:1));
    const sameTable = newTable === curTable;
    ...
    //? 3A) Si queda en la misma tabla, actualizamos SET dinámico
    if (sameTable) {
      const sets = [];
      ...
      if (Pass && password) {
        ...
        sets.push(`${esc(Pass)}=@hash`);
        reqq.input("hash", MSSQL.NVarChar, hash);
      }
    ...
    //? 3B) Si cambia de tabla → INSERT en destino y DELETE en origen (migración)
    //* Cargar metadata de tabla destino
    const destCols = await getCols(pool, newTable);
    ...
    //* valores finales a insertar (body tiene prioridad, sino lo que ya había)
    ...
    if (dEmail && vCorreo)  { insCols.push(dEmail);  insVals.push("@correo"); insReq.input("correo", MSSQL.NVarChar, vCorreo); }
    if (dNom)               { insCols.push(dNom);    insVals.push("@nombre"); insReq.input("nombre", MSSQL.NVarChar, vNombre || ""); }
    if (dApe)               { insCols.push(dApe);    insVals.push("@apellido"); insReq.input("apellido", MSSQL.NVarChar, vApe || ""); }
    if (dNomina)            { insCols.push(dNomina); insVals.push("@nomina"); insReq.input("nomina", MSSQL.NVarChar, vNomina || ""); }
    if (dRFID)              { insCols.push(dRFID);   insVals.push("@rfid");   insReq.input("rfid", MSSQL.NVarChar, vRFID || ""); }

    //* password: si mandan una nueva la usamos, si no y existe col de pass en destino, NO migramos la vieja hash por seguridad (requiere reset manual)
    if (dPass && body.password) {
      const hash = await bcrypt.hash(body.password, 10);
      insCols.push(dPass); insVals.push("@hash"); insReq.input("hash", MSSQL.NVarChar, hash);
    }
    //* rol en destino:
    const targetRol = decideTargetTable(rol) === USERS_TABLE ? (rol ?? 1) : 3;
    if (dRol) { insCols.push(dRol); insVals.push("@rol"); insReq.input("rol", MSSQL.Int, Number(targetRol)); }

    if (!insCols.length) {
      return NextResponse.json({ ok:false, error:"No hay columnas para insertar en destino" }, { status:400 });
    }

    //* Ejecutar dentro de una transacción simple (best-effort)
    const txn = pool.transaction();
    await txn.begin();

    try {
      await new MSSQL.Request(txn)
        .input("correo", insReq.parameters["correo"]?.value ?? undefined)
        .input("nombre", insReq.parameters["nombre"]?.value ?? undefined)
        .input("apellido", insReq.parameters["apellido"]?.value ?? undefined)
        .input("nomina", insReq.parameters["nomina"]?.value ?? undefined)
        .input("rfid", insReq.parameters["rfid"]?.value ?? undefined)
        .input("hash", insReq.parameters["hash"]?.value ?? undefined)
        .input("rol", insReq.parameters["rol"]?.value ?? undefined)
        .query(`
          INSERT INTO ${esc(SCHEMA)}.${esc(newTable)} (${insCols.map(esc).join(", ")})
          VALUES (${insVals.join(", ")})
        `);
