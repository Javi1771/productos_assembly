...
export async function GET() {
  try {
    ...
    const descriptions = (dsc.recordset || []).map((r) => String(r.v).toUpperCase());
    const dies         = (d.recordset || []).map((r) => String(r.v).toUpperCase());
    const crimps       = (c.recordset || []).map((r) => String(r.v).toUpperCase());

    return NextResponse.json({ ok: true, items, descriptions, dies, crimps });
  } catch (err) {
    console.error("[collar-a/examples][GET] error:", err);
