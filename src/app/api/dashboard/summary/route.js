...
export async function GET() {
  try {
    ...
    const perModule = {};
    MODULES.forEach(
      (m) => (perModule[m.key] = { label: m.label, count: 0, percent: 0 })
    );
    ...
    //? 4) Recientes: traer TODOS y ordenar numéricamente (Item es nchar)
  ...
  SELECT
    ...
    A.[NCI],
    A.[CustomerRev],
    COALESCE(A.[Adds], '') AS Adds
  FROM [dbo].[Assembly] AS A
  ORDER BY TRY_CONVERT(INT, NULLIF(LTRIM(RTRIM(A.[Item])),'')) DESC, A.[Item] DESC
`);
    ...
    const avgModulesPerAssembly = total
      ? +(sumModulesPerAssembly / total).toFixed(2)
      : 0;
    ...
    return NextResponse.json(
      { ok: false, error: "No se pudo cargar el resumen" },
      { status: 500 }
    );
  }
}
