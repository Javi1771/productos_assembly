// src/app/assembly/[item]/sleeve-guard/page.jsx
"use client";
...
export default function SleeveGuardPage() {
  ...
  const [item, setItem] = useState("");      // opcional; si lo ponen, va a Adds[1]
  const [sleeve, setSleeve] = useState("");  // opcional
  const [minv, setMinv] = useState("");      // obligatorio
  const [maxv, setMaxv] = useState("");      // obligatorio
  const [nom, setNom] = useState("");        // obligatorio

  const [examples, setExamples] = useState({ items: [], sleeves: [] });
  ...
  async function loadData() {
    ...
    try {
      const [rex, rcur] = await Promise.all([
        fetch("/api/sleeve/examples", { cache: "no-store" }),
        fetch(`/api/sleeve?assemblyItem=${assemblyItem}`, { cache: "no-store" }),
      ]);
      ...
      if (!rcur.ok || !dcur?.ok) throw new Error(dcur?.error || "No se pudo obtener Sleeve");

      if (dcur.sleeve) {
        // Modo edición
        setIsEditing(true);
        setItem(dcur.sleeve.Item != null ? String(dcur.sleeve.Item) : "");
        setSleeve(dcur.sleeve.Sleeve ?? "");
        setMinv(dcur.sleeve.Min != null ? String(dcur.sleeve.Min) : "");
        setNom(dcur.sleeve.Nom != null ? String(dcur.sleeve.Nom) : "");
        setMaxv(dcur.sleeve.Max != null ? String(dcur.sleeve.Max) : "");
      } else {
        setIsEditing(false);
      }
    } catch (e) {
  ...
  async function onSubmit(e) {
    ...
    // Min/Nom/Max requeridos
    if (minv === "" || maxv === "" || nom === "") {
      showWarning("Min, Max y Nom son obligatorios");
      return;
    }

    setLoading(true);
    try {
      const r = await fetch("/api/sleeve", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          assemblyItem,
          item: item === "" ? null : Number(item),
      ...
      showSuccess(`${isEditing ? "Sleeve actualizado" : "Sleeve guardado"}${d.sleeve?.item ? ` (Item ${d.sleeve.item})` : ""}.`);
  ...
  if (loadingMeta) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-950 dark:via-blue-950 dark:to-indigo-950 grid place-items-center">
        <div className="text-center">
          <div className="relative">
            ...
            <div className="absolute inset-0 w-16 h-16 mx-auto rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 animate-pulse opacity-50"></div>
          </div>
          <p className="text-slate-700 dark:text-slate-300 text-sm font-semibold">Cargando datos de Sleeve/Guard…</p>
          <p className="text-slate-500 dark:text-slate-500 text-xs mt-1">
            Preparando configuración de protección
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-slate-950 dark:via-blue-950 dark:to-indigo-950">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-80 h-80 rounded-full bg-gradient-to-br from-blue-400/15 to-indigo-600/15 blur-3xl"></div>
        <div className="absolute -bottom-40 -left-40 w-80 h-80 rounded-full bg-gradient-to-tr from-cyan-400/15 to-blue-600/15 blur-3xl"></div>
      </div>

      {/* Enhanced Topbar */}
      <header className="relative border-b border-slate-200/60 dark:border-slate-800/60 bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 text-white shadow-lg">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-bold">Sleeve / Guard Cut</h1>
                  <p className="text-white/80 text-xs">Configuración de protección y manga</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              {isEditing && (
                <div className="flex items-center gap-2 text-xs bg-blue-500/90 text-white px-3 py-2 rounded-lg border border-blue-400">
      ...
      <main className="relative max-w-6xl mx-auto px-4 py-8">
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-2xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
          ...
          <div className="relative p-6 bg-gradient-to-r from-blue-500 via-indigo-500 to-violet-500 text-white">
            <div className="absolute inset-0 bg-black/10"></div>
            <div className="relative flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-lg font-bold">
                    {isEditing ? "Editar Sleeve/Guard" : "Configurar Sleeve/Guard"}
                  </h2>
                  <p className="text-white/90 text-sm">
                    {isEditing ? "Modifica los parámetros de protección" : "Define los parámetros de sleeve y protección"}
                  </p>
                </div>
              </div>
              <div className="hidden sm:block text-xs text-white/80 bg-white/15 px-3 py-2 rounded-lg backdrop-blur-sm">
                <div className="flex items-center gap-1">
                  ...
                  <span>El <b>Item</b> se agregará a <b>Adds[2]</b> (opcional)</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={onSubmit} className="p-8 space-y-8">
            {/* Enhanced Item Field (Optional) */}
            <div className="space-y-3">
              <label className="flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-slate-300">
                <div className="w-2 h-2 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500"></div>
                Item del Sleeve/Guard
                <span className="text-xs text-slate-500 dark:text-slate-400 font-normal">(Opcional)</span>
                {isEditing && <span className="text-xs text-blue-600 dark:text-blue-400">(Solo lectura)</span>}
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <Hash className={`h-5 w-5 ${isEditing ? 'text-blue-500' : 'text-slate-500 dark:text-slate-400'}`} />
                </div>
                <input
                  value={item}
                  onChange={(e) => setItem(e.target.value.replace(/\D/g, ""))}
                  inputMode="numeric"
                  placeholder="Ej. 773929929 (Opcional)"
                  readOnly={isEditing}
                  className={`w-full pl-12 pr-4 py-4 rounded-xl border-2 text-slate-900 dark:text-slate-100
                             transition-all duration-200
                             ${isEditing 
                               ? "bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800 cursor-not-allowed" 
                               : "bg-white dark:bg-slate-950/50 border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500"
                             }`}
                />
              </div>
              <div className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400">
                <Info className="w-3 h-3" />
                <span>Si se especifica, se agregará a Adds[2]. Ejemplos: {examples.items.slice(0, 3).join(" • ") || "Cargando..."}</span>
              </div>
            </div>

            {/* Enhanced Sleeve Field (Optional) */}
            <div className="space-y-3">
              <label className="flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-slate-300">
                <div className="w-2 h-2 rounded-full bg-gradient-to-r from-violet-500 to-purple-500"></div>
                Descripción del Sleeve
                <span className="text-xs text-slate-500 dark:text-slate-400 font-normal">(Opcional)</span>
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                ...
                <input
                  value={sleeve}
                  onChange={(e) => setSleeve(e.target.value.toUpperCase())}
                  placeholder="Ej. Protector térmico, manga aislante..."
              ...
              <div className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400">
                <Info className="w-3 h-3" />
                <span>Ejemplos: {examples.sleeves.slice(0, 3).join(" • ") || "Cargando..."}</span>
              </div>
            </div>

            {/* Enhanced Required Measurements */}
              ...
              <div className="p-4 rounded-xl bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800">
                <div className="flex items-center gap-2 text-red-700 dark:text-red-400">
                  ...
                  <p className="text-sm font-medium">
                    Los campos Min, Nom y Max son obligatorios para el sleeve/guard
                  </p>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl border-2 bg-white dark:bg-slate-950/50
                               ...
                               invalid:border-red-300 dark:invalid:border-red-700"
                  />
                </div>
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-sm font-medium text-slate-600 dark:text-slate-400">
            ...
            <div className="flex justify-end pt-6">
              <button
                disabled={loading || minv === "" || maxv === "" || nom === ""}
                type="submit"
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    {isEditing ? "Actualizar Sleeve/Guard" : "Guardar Sleeve/Guard"}
        ...
        {/* Info Card */}
        <div className="mt-8 rounded-2xl border border-slate-200 dark:border-slate-800 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20 shadow-lg p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 rounded-lg bg-blue-500 text-white">
              <Shield className="w-4 h-4" />
            </div>
            <h4 className="font-bold text-slate-900 dark:text-slate-100">
              Información del Sleeve/Guard
            </h4>
          </div>
          <ul className="space-y-3 text-sm text-slate-600 dark:text-slate-400">
            <li className="flex items-start gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-2 flex-shrink-0"></div>
              <span>El campo <strong>Item</strong> es opcional. Si se especifica, se agregará como segundo elemento (<strong>Adds[2]</strong>) en el Assembly.</span>
            </li>
            <li className="flex items-start gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-2 flex-shrink-0"></div>
              <span>La <strong>descripción del Sleeve</strong> es opcional y puede incluir información sobre el tipo de protección.</span>
            </li>
            <li className="flex items-start gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-violet-500 mt-2 flex-shrink-0"></div>
              <span>Las medidas <strong>Min</strong>, <strong>Nom</strong> y <strong>Max</strong> son <em>obligatorias</em> y definen los parámetros de tolerancia del sleeve/guard.</span>
            </li>
          </ul>
        </div>
      </main>
